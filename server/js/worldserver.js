...
module.exports = World = cls.Class.extend({
    ...
    pushToPlayer: function(player, message) {
	    ...
	    log.error("pushToPlayermessage: " + message);
            log.error("pushToPlayer: player was undefined" + player);
        }
    },
      ...
      if(guild){
        if(typeof except === "undefined"){
          guild.forEachMember(function (player, id){
            var entityId = self.getEntityById(id);
            if (entityId) {
              self.pushToPlayer(entityId, message);
            }
          });
        }
        else{
        ...
        if(group) {
            _.each(group.players, function(playerId) {
                if(playerId != ignoredPlayer) {
                    var entityId = self.getEntityById(playerId);
                    if (entityId) {
                      self.pushToPlayer(entityId, message);
                    }
                }
    ...
    handleHurtEntity: function(entity, attacker, damage) {
        ...
        if(entity.hitPoints <= 0) {
            if(entity.type === "mob") {
                ...
                // get player that tanked the mob
                var mainTankerId = mob.getMainTankerId();
                if (mainTankerId != null) {
                  var mainTanker = this.getEntityById(mainTankerId);
                }
